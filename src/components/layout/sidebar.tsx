    ...
    <>
      <aside className="hidden lg:flex lg:w-60 lg:flex-col lg:border-r lg:border-border lg:bg-bone">
        <div className="flex h-16 items-center border-b border-border px-6">
          <Link href="/dashboard" className="font-serif text-xl font-semibold tracking-tight text-burgundy">
            Audiobook Engine
          </Link>
        </div>
      ...
      <header className="flex h-14 items-center justify-between border-b border-border bg-bone px-4 lg:hidden">
        <Link href="/dashboard" className="font-serif text-lg font-semibold text-burgundy">
          Audiobook Engine
        </Link>
...
function MobileNav({
  ...
  return (
    <details className="relative">
      ...
      <div className="absolute right-0 top-8 z-50 min-w-[180px] rounded-md border border-border bg-bone py-2 shadow-md">
        {navItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
